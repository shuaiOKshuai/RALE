...
subpathsSaveByIdsBatchSz = cf.getint("param", "subpathsSaveByIdsBatchSz")

start_time=time.time()
print('Start time ==', datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))

nodes_num = prepareDatasetMatch2.prepareRandomWalkAndSubpaths(
    root_dir, 
    dataset,
    split_index, 
    kshot, 
...
prepareDatasetMatch2.reprocessSubpathsAndSaveByNodeIds(
    nodes_num,
    root_dir, 
    split_index, 
    kshot, 
